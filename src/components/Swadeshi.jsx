...
const Swadeshi = () => { const [mousePos, setMousePos] = useState({ x: 0, y: 0 });

useEffect(() => { const handleMouseMove = (e) => { setMousePos({ x: e.clientX, y: e.clientY }); }; window.addEventListener("mousemove", handleMouseMove); return () => window.removeEventListener("mousemove", handleMouseMove); }, []);

return ( <div className="relative min-h-screen flex flex-col justify-center items-center text-white text-center bg-gradient-to-b from-[#0f0f0f] via-[#111] to-[#0c0c0c] overflow-hidden px-4 sm:px-8"> <motion.div className="absolute w-[450px] sm:w-[650px] h-[450px] sm:h-[650px] rounded-full bg-[#ff4e4e] opacity-20 blur-[160px] sm:blur-[220px]" style={{ transform: translate(${mousePos.x * 0.03}px, ${mousePos.y * 0.03}px), }} />
...
<motion.h1
  ...
  >
    A powerful step towards Atmanirbhar Bharat — promote, build and use homegrown tech
    that celebrates our identity, culture and innovation.
  </motion.p>
