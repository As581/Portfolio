...
// --- Swadeshi: Ultra WOW Section ---
// One-file, production-ready hero section for your portfolio.
// Tailwind + Framer Motion. No external images needed.
// Drop-in usage: <Swadeshi />

const floaters = [
  { label: "Made in 🇮🇳", x: -220, y: -120 },
  { label: "Local > Global", x: 200, y: -80 },
  { label: "Aatmanirbhar", x: -240, y: 90 },
  { label: "Desi Innovation", x: 210, y: 130 },
];

const FeatureCard = ({ title, desc, i }) => (
  <motion.div
    className="relative group rounded-2xl p-6 bg-white/5 border border-white/10 overflow-hidden shadow-xl backdrop-blur-sm"
...
const ShineButton = ({ children, variant = "primary" }) => (
  <motion.button
    ...
    className={
      variant === "primary"
        ? "relative px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold text-black bg-gradient-to-r from-[#ffb300] to-[#ff7a00] shadow-[0_10px_30px_rgba(255,140,0,0.25)] overflow-hidden"
        : "relative px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold border border-[#ffb300] text-[#ffb300] bg-black/20 backdrop-blur overflow-hidden"
    }
  >
...
const Swadeshi = () => {
  ...
  // Smooth parallax factors
  const glow = useMemo(() => ({ x: mouse.x * 0.02, y: mouse.y * 0.02 }), [mouse]);

  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-b from-[#0b0b0b] via-[#0a0a0a] to-[#000] text-white">
      {/* Aurora / Nebula */}
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -top-32 left-1/2 h-[70vmax] w-[70vmax] -translate-x-1/2 rounded-full blur-[120px]"
        style={{
          background:
            "radial-gradient(closest-side, rgba(255,78,78,0.35), transparent 60%), conic-gradient(from 180deg at 50% 50%, rgba(255,179,0,0.2), transparent 40%, rgba(255,78,78,0.18))",
        }}
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-8 pt-28 pb-16">
        ...
        >
          <motion.h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-[#ff4e4e] via-[#ff8a00] to-[#ffb300] drop-shadow-[0_0_30px_rgba(255,140,0,0.25)]">
          ...
          >
            "स्वदेशी भावेन नव-भारत निर्माणम्।" — Create, adopt and champion Indian-made tech that carries our heritage forward.
          </motion.p>
        ...
        <div className="mt-14 grid grid-cols-2 sm:grid-cols-4 gap-5 text-center">
          {["Creators", "Projects", "Downloads", "Cities"].map((k, i) => (
            ...
            >
              <div className="text-2xl sm:text-3xl font-extrabold text-[#ffb300]">
                {["10k+", "150+", "1M+", "120+"][i]}
              </div>
